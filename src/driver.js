...
function getJs(page, technologies = WhatsOnThisSite.technologies) {
...
function analyzeJs(js, technologies = WhatsOnThisSite.technologies) {
  return js
...
function getDom(page, technologies = WhatsOnThisSite.technologies) {
  return page.evaluate((technologies) => {
    return technologies
...
function analyzeDom(dom, technologies = WhatsOnThisSite.technologies) {
  return dom
...
class Site {
  ...
  async onDetect(url, detections = []) {
    ...
    if (this.cache[url.href]) {
      ...
      const requires = [
        ...WhatsOnThisSite.requires.filter(({ name }) =>
          resolved.some(({ name: _name }) => _name === name),
        ),
        ...WhatsOnThisSite.categoryRequires.filter(({ categoryId }) =>
