...
const WhatsOnThisSite = {
  ...
  getTechnology: (name) =>
    [
      ...WhatsOnThisSite.technologies,
      ...WhatsOnThisSite.requires
        .map(({ technologies }) => technologies)
        .flat(),
      ...WhatsOnThisSite.categoryRequires
        .map(({ technologies }) => technologies)
        .flat(),
    ].find(({ name: _name }) => name === _name),

  getCategory: (id) =>
    WhatsOnThisSite.categories.find(({ id: _id }) => id === _id),
  ...
  resolve(detections = []) {
    ...
    WhatsOnThisSite.resolveExcludes(resolved);
    WhatsOnThisSite.resolveImplies(resolved);

    const priority = ({ technology: { categories } }) =>
      categories.reduce(
        (max, id) => Math.max(max, WhatsOnThisSite.getCategory(id).priority),
        0,
      );
    ...
    return resolved
      ...
      .map(
        ({
          ...
          description,
          slug,
          categories: categories.map((id) => WhatsOnThisSite.getCategory(id)),
          confidence,
          version,
          icon,
          website,
  ...
  resolveExcludes(resolved) {
    resolved.forEach(({ technology }) => {
      technology.excludes.forEach(({ name }) => {
        const excluded = WhatsOnThisSite.getTechnology(name);
      ...
      resolved.forEach(({ technology, confidence, lastUrl }) => {
        technology.implies.forEach(
          ({ name, confidence: _confidence, version }) => {
            const implied = WhatsOnThisSite.getTechnology(name);
   ...
   * @param {*} param0
   */
  analyze(items, technologies = WhatsOnThisSite.technologies) {
    const oo = WhatsOnThisSite.analyzeOneToOne;
    const om = WhatsOnThisSite.analyzeOneToMany;
    const mm = WhatsOnThisSite.analyzeManyToMany;
  ...
  setTechnologies(data) {
    const transform = WhatsOnThisSite.transformPatterns;

    WhatsOnThisSite.technologies = Object.keys(data).reduce(
      (technologies, name) => {
        const {
          cats,
          certIssuer,
        ...
        technologies.push({
          ...
          slug: WhatsOnThisSite.slugify(name),
          text: transform(text),
          url: transform(url),
    ...
    WhatsOnThisSite.technologies
      ...
      .forEach((technology) =>
        technology.requires.forEach(({ name }) => {
          if (!WhatsOnThisSite.getTechnology(name)) {
            throw new Error(`Required technology does not exist: ${name}`);
          }

          WhatsOnThisSite.requires[name] = WhatsOnThisSite.requires[name] || [];

          WhatsOnThisSite.requires[name].push(technology);
        }),
      );

    WhatsOnThisSite.requires = Object.keys(WhatsOnThisSite.requires).map(
      (name) => ({
        name,
        technologies: WhatsOnThisSite.requires[name],
      }),
    );
    ...
    WhatsOnThisSite.technologies
      ...
      .forEach((technology) =>
        technology.requiresCategory.forEach(({ id }) => {
          WhatsOnThisSite.categoryRequires[id] =
            WhatsOnThisSite.categoryRequires[id] || [];

          WhatsOnThisSite.categoryRequires[id].push(technology);
        }),
      );

    WhatsOnThisSite.categoryRequires = Object.keys(
      WhatsOnThisSite.categoryRequires,
    ).map((id) => ({
      categoryId: parseInt(id, 10),
      technologies: WhatsOnThisSite.categoryRequires[id],
    }));

    WhatsOnThisSite.technologies = WhatsOnThisSite.technologies.filter(
   ...
   */
  setCategories(data) {
    WhatsOnThisSite.categories = Object.keys(data)
      .reduce((categories, id) => {
        const category = data[id];

        categories.push({
          id: parseInt(id, 10),
          slug: WhatsOnThisSite.slugify(category.name),
          ...category,
        });
  ...
  transformPatterns(patterns, caseSensitive = false, isRegex = true) {
    ...
    const parsed = Object.keys(patterns).reduce((parsed, key) => {
      parsed[caseSensitive ? key : key.toLowerCase()] = toArray(
        patterns[key],
      ).map((pattern) => WhatsOnThisSite.parsePattern(pattern, isRegex));
  ...
  parsePattern(pattern, isRegex = true) {
    if (typeof pattern === 'object') {
      return Object.keys(pattern).reduce(
        (parsed, key) => ({
          ...parsed,
          [key]: WhatsOnThisSite.parsePattern(pattern[key]),
        }),
        {},
      );
    } else {
    ...
    return technology[type].reduce((technologies, pattern) => {
      ...
      if (matches) {
        technologies.push({
          ...
          version: WhatsOnThisSite.resolveVersion(pattern, value),
        });
      }
...
if (typeof module !== 'undefined') {
  module.exports = WhatsOnThisSite;
}
